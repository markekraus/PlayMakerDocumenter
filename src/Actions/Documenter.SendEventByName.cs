...
{
    private static StringBuilder DocActionSendEventByName(this StringBuilder sb, SendEventByName action, ActionContext ctx) =>
        action is null
        ? sb
        : sb.AppendHeader($"{nameof(SendEventByName)} Details:")
            .NewTable()
